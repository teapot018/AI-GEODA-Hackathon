...
다만 **`/user/trade` 가 주는 값은 추정가가 아니라 실제로 체결된 거래가**입니다.
다른 시세 사이트와의 차이는 값이 진짜냐가 아니라 **표본이 얼마나 넓으냐**였습니다.
그래서 조회할 때마다 체결 기록을 프로세스 메모리 풀에 합쳐, 조회가 누적될수록
카드 커버리지가 넓어지게 했습니다 (`market/pool.ts`, `market/livefeed.ts`).

| | 1회 조회 | 2회 | 3회 |
| --- | --- | --- | --- |
| 누적 체결 | 200건 | 400건 | 600건 |
| 카드 종수 | 23종 | 45종 | 64종 |
...
패널과 변동 카드에만 반영돼, "조회할수록 시세가 촘촘해진다" 는 화면의 약속을
...
다만 위쪽 **매입·매도 총액 타일은 언제나 `account`** 입니다. 그건 시세가 아니라
그 계정의 현금 흐름이라, 다른 계정 관측을 섞으면 뜻이 달라집니다.

### 넥슨 공시 기준가 대조 (`market/datacenter.ts`)

체결가 옆에 **넥슨이 2시간 주기로 집계·공시하는 기준가**를 나란히 놓고 비교합니다.

| | 출처 | 성격 | 커버리지 |
| --- | --- | --- | --- |
| **체결가** | `/user/trade` | 실제 거래가 | 조회한 계정 (풀로 누적) |
| **기준가** | 데이터센터 공개 페이지 | 2시간 집계값 | 전 카드 |

둘은 서로를 **대체하지 않습니다.** 어긋나는 폭 자체가 정보입니다 — 체결가가 기준가보다
...
- 실패해도 throw 하지 않습니다. 기준가는 체결가를 보조하는 참고치라, 못 가져왔다고
  화면이 비면 안 됩니다.
