...
describe('parseBP', () => {
   ...
   * ── 적대적 표기 ──────────────────────────────────────────
   *
   * 이 파서는 실물 페이지로 검증되지 않았다(PARSER_VERIFIED = false).
   * 그러니 "읽히는가" 보다 **"틀린 값을 자신 있게 내놓지 않는가"** 가
   * 더 중요하다. null 이면 화면은 "기준가를 읽지 못했다" 로 끝나지만,
   * 10배 틀린 값은 그대로 기준가로 표시되고 관측 비교에까지 흘러간다.
   */

  it('소수로 적힌 축약을 자릿수째로 잘라 먹지 않는다', () => {
    // 조각을 따로 긁던 시절에는 "0.5억" 에서 '5억' 이 걸려
    // 50,000,000 대신 500,000,000 이 나왔다 — 정확히 10배.
    expect(parseBP('0.5억')).toBe(50_000_000);
    expect(parseBP('1.2억')).toBe(120_000_000);
    expect(parseBP('3.75억')).toBe(375_000_000);
    expect(parseBP('12.5만')).toBe(125_000);
  });

  it('조 단위를 통째로 흘리지 않는다', () => {
    // "1조 2,345억" 에서 조를 모르면 2,345억만 남아 1/6 이 된다.
    expect(parseBP('1조 2,345억')).toBe(1_234_500_000_000);
    expect(parseBP('3조')).toBe(3_000_000_000_000);
  });

  it('Number 가 정확히 셀 수 없는 값은 읽지 않는다', () => {
    // 9,007,199,254,740,993 은 Number 로 담으면 ...992 가 된다.
    // 조용히 다른 숫자가 되느니 못 읽었다고 한다.
    expect(parseBP('9007199254740993')).toBeNull();
    expect(parseBP('99,999,999,999,999,999')).toBeNull();
    expect(parseBP(String(Number.MAX_SAFE_INTEGER))).toBe(Number.MAX_SAFE_INTEGER);
  });

  it('우리가 못 읽는 자릿수 표기는 포기한다', () => {
    /*
     * 천·백·십은 앞 단위를 꾸미는 말이라 더할 수 없다. "2억 3천만" 은
     * 230,000,000 인데, 조각을 더하면 200,003,000 이 나온다. 반쯤 읽은
     * 값을 내놓느니 null 을 낸다.
     */
    expect(parseBP('2억 3천만')).toBeNull();
    expect(parseBP('5천')).toBeNull();
    expect(parseBP('1경')).toBeNull();
  });

  it('같은 자릿수가 다시 나오면 거기서 끊는다', () => {
    // "1억 2,345만" 뒤에 붙은 다른 금액을 값에 더해 버리면 안 된다.
    expect(parseBP('1억 2,345만 (수수료 40만)')).toBe(123_450_000);
  });

  it('자릿수는 큰 것에서 작은 것으로만 내려간다', () => {
    // "2,345만 1억" 은 한 금액이 아니다. 앞 덩어리만 읽는다.
    expect(parseBP('2,345만 1억')).toBe(23_450_000);
  });

  it('단위 없는 짧은 수는 값으로 보지 않는다', () => {
    // 본문에 지나가는 "12" 를 기준가로 읽으면 안 된다.
    expect(parseBP('12')).toBeNull();
    expect(parseBP('순위 7')).toBeNull();
  });
});
