 ...
 *
 * 그래서 이 앱은 이렇게 나눈다:
 *   - 카드 목록 / 이름 / 시즌 / 이미지 → 넥슨 공식 메타 (실데이터)
 *   - 능력치 / 오버롤 / 가치         → 이 시드 파일 + 추정 모델
 *
 * 조인 키는 **선수 이름**이다. spid.json 의 한글 표기와 아래 name 이
 * 일치하면 실측 스탯이 붙고, 없으면 estimateProfile() 이 포지션 기반
 * 추정치를 만들어 넣는다. 실서비스에서는 이 파일을 크롤링/자체 DB 로
 * 교체하면 나머지 코드는 손댈 필요가 없다.
 */
