...
export const PACK_BOXES: PackBox[] = [
  {
     ...
     * seasons.ts 의 ICON/레전드 valueMultiplier 를 올려(+1 카드도 억 단위
     * 시장가처럼 보이게) 카드 기대값이 전반적으로 뛰었고, 그 결과 이 상자의
     * 기대값이 옛 가격(6,300,000)을 넘어섰다. 확률형 상품은 기대값이
     * 가격보다 낮은 게 성립 조건이라(그래야 상자를 파는 쪽이 남는다),
